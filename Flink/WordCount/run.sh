...
do
    ...
    command="java -cp target/WordCount-1.0.jar WordCount.WordCount --rate 0 --sampling 100 --parallelism $config --chaining"
    # command="ls"
    ...
    echo "CPU: $avg_cpu%"
    echo "MEM: $avg_mem%"
    echo "Flink_WC_$config, $throughput, $latency, $avg_cpu%, $avg_mem%" >> "../../results.csv" 
