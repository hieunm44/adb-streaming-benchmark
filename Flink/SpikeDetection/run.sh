...
configs=("4 4 4 4" "7 7 7 7")

for config in "${configs[@]}"
do
    echo "Config: $config"
    # Get the command from the command-line arguments
    command="java -cp target/SpikeDetection-1.0.jar SpikeDetection.SpikeDetection --rate 0 --sampling 100 --parallelism $config --chaining"
    # command="ls"
    ...
    echo "CPU: $avg_cpu%"
    echo "MEM: $avg_mem%"
    echo "Flink_SD_$config, $throughput, $latency, $avg_cpu%, $avg_mem%" >> "../../results.csv" 
